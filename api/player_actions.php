...
if ($action === 'submit_answer') {
    ...
    if ($player['current_question_id'] != $questionId || $player['session_status'] !== 'question') {
        echo json_encode(['status' => 'error', 'message' => 'Question is not active or time is up']);
        exit;
    }
    ...
    if ($stmt->fetch()) {
        echo json_encode(['status' => 'error', 'message' => 'You have already submitted an answer for this question']);
        exit;
    }
    ...
    if (!$question || !$answer) {
        echo json_encode(['status' => 'error', 'message' => 'Invalid question or answer']);
        exit;
    }
